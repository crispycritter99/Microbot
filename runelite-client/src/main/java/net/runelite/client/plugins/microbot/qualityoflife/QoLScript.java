...
public class QoLScript extends Script {
    ...
    public boolean run(QoLConfig config) {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if (QoLLocalPlugin.executeBankActions) {
                    handleBankActions();
                }

                if (QoLLocalPlugin.executeFurnaceActions) {
                    handleFurnaceActions();
                }

                if (QoLLocalPlugin.executeAnvilActions) {
                    handleAnvilActions();
                }

                if (QoLLocalPlugin.executeWorkbenchActions) {
                    handleWorkbenchActions();
                }

                if (QoLLocalPlugin.executeLoadoutActions && QoLLocalPlugin.loadoutToLoad != null) {
    ...
    private void handleInventorySetup() {
        if (!openBank()) {
            ...
            QoLLocalPlugin.executeLoadoutActions = false;
            QoLLocalPlugin.loadoutToLoad = null;
            return;
        }

        try {
            Rs2InventorySetup inventorySetup = new Rs2InventorySetup(QoLLocalPlugin.loadoutToLoad.getName(), mainScheduledFuture);
    ...
    private void handleWorkbenchActions() {
        ...
        Microbot.doInvoke(QoLLocalPlugin.workbenchMenuEntry, new Rectangle(1, 1));
        QoLLocalPlugin.executeWorkbenchActions = false;
    }

    private void handleBankActions() {
        if (!openBank()) {
            log.warn("Bank did not open");
            QoLLocalPlugin.executeBankActions = false;
            return;
        }

        for (NewMenuEntry menuEntry : QoLLocalPlugin.bankMenuEntries) {
    ...
    private void handleFurnaceActions() {
        if (!openFurnace()) {
            ...
            QoLLocalPlugin.executeFurnaceActions = false;
            return;
        }

        for (NewMenuEntry menuEntry : QoLLocalPlugin.furnaceMenuEntries) {
    ...
    private void handleAnvilActions() {
        if (!openAnvil()) {
            log.warn("Production widget did not open");
            QoLLocalPlugin.executeAnvilActions = false;
            return;
        }

        for (NewMenuEntry menuEntry : QoLLocalPlugin.anvilMenuEntries) {
    ...
    public void resetMenuEntries() {
        QoLLocalPlugin.bankMenuEntries.clear();
        QoLLocalPlugin.furnaceMenuEntries.clear();
        QoLLocalPlugin.anvilMenuEntries.clear();
    }
