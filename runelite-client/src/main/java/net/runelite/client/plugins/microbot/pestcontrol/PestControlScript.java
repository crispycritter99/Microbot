...
public class PestControlScript extends Script {
    ...
    private final PestControlLocalPlugin plugin;
    public static Pair<Portal, Integer> closestPortal= null;
    @Inject
    public PestControlScript(PestControlLocalPlugin plugin, PestControlConfig config) {
        this.plugin = plugin;
        this.config = config;
    }
...
//                        sleepUntil(() -> !Rs2Combat.inCombat());
    ...
    public Portal getClosestAttackablePortal() {
        ...
        if (distancesToPortal.isEmpty()){
            for (Portal portal : portals) {
                if (!portal.getHitPoints().getText().trim().equals("0")) {
                    distancesToPortal.add(Pair.of(portal, distanceToRegion(portal.getRegionX(), portal.getRegionY())));
                }
            }
            if (distancesToPortal.size()>1){
                distancesToPortal = new ArrayList();
            }
        }
...
//            if (!portal.isHasShield() && !portal.getHitPoints().getText().trim().equals("0") && closestAttackablePortal == portal) {
            if (closestAttackablePortal == portal) {
                if (!Rs2Walker.isCloseToRegion(distanceToPortal, portal.getRegionX(), portal.getRegionY())) {
                    Rs2Walker.walkTo(WorldPoint.fromRegion(Rs2Player.getWorldLocation().getRegionID(), portal.getRegionX(), portal.getRegionY(), Microbot.getClient().getTopLevelWorldView().getPlane()), 10);
                    attackPortal();
                } else {
                    attackPortal();
                }
