...
import static net.runelite.client.plugins.microbot.temporossSolo.State.getAllFish;
import static net.runelite.client.plugins.microbot.temporossSolo.State.getTotalAvailableFishSlots;
...
public class TemporossProgressionOverlay extends OverlayPanel {
    ...
    @Inject
    public TemporossProgressionOverlay(TemporossSoloPlugin plugin) {
        super(plugin);
        this.plugin = plugin;
    ...
    private double calculateProgression(State state) {
        switch (state) {
            ...
            case SECOND_FILL:
                ...
                return 1.0 - Math.min((double) cookedFishSecondFill / (TemporossScript.temporossSoloConfig.solo() ? 19 : getTotalAvailableFishSlots()), 1.0);

            case INITIAL_FILL:
                // Progression goes up as cooked fish decreases (0 fish = 100% progress).
                int cookedFishInitialFill = State.getCookedFish();
                return 1.0 - Math.min((double) cookedFishInitialFill / (TemporossScript.temporossSoloConfig.solo() ? 17 : getTotalAvailableFishSlots()), 1.0);

            case THIRD_COOK:
                // Progression based on cooked fish count or intensity threshold.
                int cookedFishThirdCook = State.getCookedFish();
                return Math.min((double) cookedFishThirdCook / (TemporossScript.temporossSoloConfig.solo() ? 19 : getAllFish()), 1.0);

            case THIRD_CATCH:
                // Progression based on total fish count, target is 19.
                int allFishThirdCatch = getAllFish();
                return Math.min((double) allFishThirdCatch / (TemporossScript.temporossSoloConfig.solo() ? 19 : getTotalAvailableFishSlots()), 1.0);
            ...
            case SECOND_COOK:
                ...
                return Math.min((double) cookedFishSecondCook / (TemporossScript.temporossSoloConfig.solo() ? 17 : getAllFish()), 1.0);

            case SECOND_CATCH:
                // Progression based on total fish count, target is 17.
                int allFishSecondCatch = getAllFish();
                return Math.min((double) allFishSecondCatch / (TemporossScript.temporossSoloConfig.solo() ? 17 : getTotalAvailableFishSlots()), 1.0);
