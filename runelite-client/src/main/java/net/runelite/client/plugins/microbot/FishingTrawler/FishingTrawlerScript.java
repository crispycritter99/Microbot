            ...
            try {
                ...
                if (Rs2GameObject.exists(2483)) {
                    if (lootnet) {
                        ...
                        Microbot.status="Looting Reward";
                    ...
                    if (Rs2Player.getWorldLocation().getPlane() == 0&&Rs2GameObject.interact(new WorldPoint(2675, 3170, 0), "Cross")) {
                        ;
                        Microbot.status="Boarding Ship";
...
//                if contribution.isEmpty()
                int ContributionValue = !contribution.isEmpty()?Integer.parseInt(contribution.split(":")[1].strip()):0;

                if (Rs2Player.getWorldLocation().getPlane() == 0 && Rs2GameObject.exists(4060)) {
                    Microbot.status="Getting into Position";
                    lootnet = true;
                    sleep(Rs2Random.randomGaussian(1200, 300));
                    ...
                    Rs2Walker.walkFastCanvas(new WorldPoint(ladder.dx(1).getX(), 4827, 1));
                }
                ...
                if (!Rs2Player.isInteracting() && !Rs2Player.isMoving() && Rs2Npc.getNpc("Enormous Tentacle") != null) {
                    Microbot.status="Chopping Tentacle";
                    awaitExecutionUntil(() -> {
                 ...
                 if (!Rs2Player.isInteracting() && !Rs2Player.isMoving() && Rs2Npc.getNpc("Enormous Tentacle") == null && tentacle) {
                     Microbot.status="Moving after Chopping Tentacle";
