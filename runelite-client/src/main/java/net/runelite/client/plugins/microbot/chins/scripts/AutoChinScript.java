package net.runelite.client.plugins.microbot.chins.scripts;
...
import net.runelite.client.plugins.microbot.chins.ChinHunterConfig;
import net.runelite.client.plugins.microbot.chins.ChinHunterPlugin;

import net.runelite.client.plugins.microbot.chins.HunterTrap;
...
public class AutoChinScript extends Script {
    ...
    public boolean run(ChinHunterConfig config) {
    ...
    private void handleIdleState() {
        try {
            ...
            ChinHunterPlugin.getTraps();
            List<HunterTrap> traps = new ArrayList<>(ChinHunterPlugin.getTraps().values());
...
//            }
    ...
    private void handleDroppingState(ChinHunterConfig config) {
        sleep(config.minSleepAfterLay(), config.maxSleepAfterLay());
        currentState = State.IDLE;
    }

    private void handleCatchingState(ChinHunterConfig config) {
...
//        sleepUntil(()->Rs2GameObject.getGameObjects(9380).size()!=prevtraps,7500);
    ...
    private void handleLayingState(ChinHunterConfig config) {
        sleep(config.minSleepAfterLay(), config.maxSleepAfterLay());
        currentState = State.IDLE;
    }
