...
public class NavigationHandler {
     ...
     */
    public static boolean navigateToTotem(TotemLocation totemLocation, net.runelite.client.plugins.microbot.valetotemsbeta.models.GameSession gameSession) {
        try {
            ...
            while (!CoordinateUtils.isAtTotemLocation(totemLocation, ARRIVAL_TOLERANCE) &&
                ...
                // A second walk/transport click cancels the processing action. Let the current
                // movement segment carry the player while fletching, then resume pathing once it ends.
                if (FletchingHandler.isFletchingWhileWalking()) {
                ...
                // Priority 3: Begin a batch while already moving, then keep navigating.
                if (CoordinateUtils.getPlayerLocation().distanceTo(nextCheckpoint) > 13) {
                    ...
                    if (justWalkedOverEntTrails) {
                        ...
                        int remainingTotems = totemLocation.getRouteType().getMaxTotems() - totemLocation.getOrder() + 1;
                        int needed = Math.min(InventoryUtils.getLogCount(),
                                Math.max(0, remainingTotems * 4 - InventoryUtils.getBowCount()));
                        if (FletchingHandler.startFletchingWhileWalking(needed, gameSession)) {
                            continue;
                        }
                    }
                }
     ...
     * Processes every distinct nearby pair; processed coordinates prevent repeat attempts.
    ...
    public static boolean checkAndWalkOverEntTrails() {
        try {
            ...
            if (firstVisited && secondVisited) {
                ...
                System.out.println("Ent trail pair completed; continuing to scan for new pairs");
                    ...
                    if (allTransports.containsKey(pathPoint)) {
                        ...
                        for (net.runelite.client.plugins.microbot.shortestpath.Transport transport : allTransports.get(pathPoint)) {
                            if (transport.getType().toString().contains("AGILITY") &&
                                ...
                                // The southern Auburn Valley balance saves one tick but removes useful
                                // moving-fletching time, so the strategy route deliberately avoids it.
                                if (transport.getOrigin().distanceTo(new WorldPoint(1401, 3287, 0)) <= 2) {
                                    continue;
                                }
