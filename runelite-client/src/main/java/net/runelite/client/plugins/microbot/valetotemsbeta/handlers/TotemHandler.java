...
import net.runelite.client.plugins.microbot.valetotemsbeta.enums.SpiritAnimal;
...
public class TotemHandler {
    ...
    private static boolean waitForPreviousTotemToFinish(TotemLocation currentTotemLocation) {
        ...
        if (currentState != GameObjectId.TOTEM_SITE && currentState != null) {
            System.out.println("Waiting for current totem at " + currentTotemLocation.getDescription() + " to finish");
            if (!sleepUntil(() -> {
                GameObjectUtils.invalidateCacheAtLocation(currentTotemLocation.getLocation());
                return checkTotemState(currentTotemLocation) == GameObjectId.TOTEM_SITE;
            }, 300000)) {
