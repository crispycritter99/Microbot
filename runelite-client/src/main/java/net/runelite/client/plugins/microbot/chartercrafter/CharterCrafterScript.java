...
public class CharterCrafterScript extends Script {
    ...
    private void worldHop() {
        ...
        System.out.println("Hopping world, please wait");

        for (int attempt = 0; attempt < maxAttempts; attempt++) {
            final int currentWorld = Microbot.getClient().getWorld();
            Microbot.hopToWorld(Login.getNextWorld(Rs2Player.isMember()));
            ...
            if (changed) {
                System.out.println("Successful hop world");
                return;
            }
            sleep(250, 350);
            System.out.println("Hop attempt "+attempt+"/"+maxAttempts+", retrying...");
        }
        System.out.println("Failed to hop world");
    }
