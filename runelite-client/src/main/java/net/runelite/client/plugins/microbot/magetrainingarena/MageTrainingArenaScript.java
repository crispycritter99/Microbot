...
public class MageTrainingArenaScript extends Script {
            ...
            try {
                ...
                if (mtaPlugin == null) {
                    ...
                    mtaPlugin = (MTAPlugin) Microbot.getPluginManager().getPlugins()
                            .stream().filter(x -> x instanceof MTAPlugin)
    ...
    private void handleEnchantmentRoom() {
        ...
        if (Rs2Inventory.contains(ItemID.MAGICTRAINING_DRAGONSTONE) || Rs2Inventory.count(itemId) >= shapesToPick) {
            shapesToPick = Rs2Random.between(1, 3);
    ...
    private void handleTelekineticRoom() {
        ...
        if (room.getTarget() != null)
            ...
            target = room.getTarget();
            var nextTarget = TelekineticRoom.optimal(1);
            sleep(400, 600);
        }
...
//                if Rs2Magic.isSpellSelected()
                ...
                if (Microbot.getClient().isWidgetSelected())
                {
                if (Rs2Npc.interact(new Rs2NpcModel(room.getGuardian()))&&TelekineticRoom.getMoves().size() >= 2);

                        var nextTarget = LocalPoint.fromWorld(Microbot.getClient().getTopLevelWorldView(), TelekineticRoom.optimal(1));
                        var nextTargetConverted = WorldPoint.fromLocalInstance(Microbot.getClient(), Objects.requireNonNull(nextTarget));
//                        WorldPoint destination = WorldPoint.fromLocalInstance(Microbot.getClient(), Objects.requireNonNull(room.getDestination()));
                        sleepGaussian(600, 150);
                        Rs2Walker.walkFastCanvas(nextTargetConverted);
                        sleepGaussian(600, 150);
                        Rs2Magic.cast(MagicAction.TELEKINETIC_GRAB);
//                        Rs2Npc.hoverOverActor(new Rs2NpcModel(room.getGuardian()));
//                        room.getDestination();
                }
