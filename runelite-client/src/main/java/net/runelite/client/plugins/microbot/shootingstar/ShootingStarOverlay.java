...
public class ShootingStarOverlay extends OverlayPanel
{
	...
	private final ShootingStarLocalPlugin plugin;
	private final ShootingStarConfig config;
	private final ImageComponent image;
	private final DateTimeFormatter dtf = DateTimeFormatter.ofPattern("HH:mm:ss");

	@Inject
	ShootingStarOverlay(ShootingStarLocalPlugin plugin, ShootingStarConfig config)
	{
		super(plugin);
		this.plugin = plugin;
		this.config = config;
	...
	public Dimension render(Graphics2D graphics)
	{
		try
		{
			...
			final LineComponent titleLine = LineComponent.builder()
				.left(" ShootingStar Miner V" + ShootingStarLocalPlugin.version)
	...
	private BufferedImage getCrashedStarImage()
	{
		try
		{
			var img = ImageUtil.loadImageResource(ShootingStarLocalPlugin.class, "crashed_star_7.png");
