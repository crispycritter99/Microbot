...
public class BrimhavenSpikeJumperScript extends Script {
    ...
    public static boolean lootnet = false;
     boolean test = false;
    public boolean run(BrimhavenSpikeJumperConfig config) {
...
//                        if (item.getItem().getName().contains("bones")||item.getItem().getName().contains("sheet")||item.getItem().getName().contains("Dra")||item.getItem().getName().contains("Run")) {
//                            double LOG_MEAN = 0.05; double LOG_STD = 0.34;Random r = new Random();double gaussian = r.nextGaussian();
//                            double value = Math.exp(LOG_MEAN + LOG_STD * gaussian);
//                            sleep((int) value*400);
...
//                }
                //BLAST FURNACE
//                                if (Rs2Widget.findWidget("How many would you like to take?", null) != null){
//                    double LOG_MEAN = 0.25; double LOG_STD = 0.34;Random r = new Random();double gaussian = r.nextGaussian();
//                    double value = Math.exp(LOG_MEAN + LOG_STD * gaussian);
//                    sleep((int) value*200);
//                    Rs2Keyboard.keyPress(KeyEvent.VK_SPACE);
...
//                }
                WorldPoint tile = new WorldPoint(2799, 9568, 3);
                if (tile.distanceTo(Rs2Player.getWorldLocation())<5&&Rs2Player.getHealthPercentage()>10)
                {
                    double initialHealth = Rs2Player.getHealthPercentage();
                    int initialxp = Microbot.getClient().getSkillExperience(Skill.AGILITY);
                    double LOG_MEAN = 0.20; double LOG_STD = 0.4;Random r = new Random();double gaussian = r.nextGaussian();
                    double value = Math.exp(LOG_MEAN + LOG_STD * gaussian);
                    sleep((int) value*200);
                    Rs2Walker.walkFastCanvas(tile);
//                    Rs2Player.waitForXpDrop(Skill.AGILITY);
                    sleepUntil(()->initialHealth<Rs2Player.getHealthPercentage()||initialxp<Microbot.getClient().getSkillExperience(Skill.AGILITY));
                    LOG_MEAN = 0.20; LOG_STD = 0.4;r = new Random(); gaussian = r.nextGaussian();
                    value = Math.exp(LOG_MEAN + LOG_STD * gaussian);
                    sleep((int) value*200);
                }
                Rs2Player.eatAt(10);
