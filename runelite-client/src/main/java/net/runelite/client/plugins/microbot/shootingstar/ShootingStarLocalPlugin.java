...
@PluginDescriptor(
	name = PluginConstants.SHOOTING_STAR_PREFIX + "ShootingStar Local",
	description = "Finds & Travels to shooting stars",
	authors = { "g-mason0" },
	version = ShootingStarLocalPlugin.version,
...
@Slf4j
public class ShootingStarLocalPlugin extends Plugin
{
	...
	private void createPanel()
	{
		if (panel == null)
		{
			...
			final BufferedImage icon = ImageUtil.loadImageResource(ShootingStarLocalPlugin.class, "icon.png");
	...
	public Star getClosestHighestTierStar()
	{
		...
		Pathfinder pathfinder = new Pathfinder(ShortestPathPlugin.getPathfinderConfig(), Rs2Player.getLocalPlayer().getWorldLocation(), accessibleStarPoints);
		pathfinder.run();
