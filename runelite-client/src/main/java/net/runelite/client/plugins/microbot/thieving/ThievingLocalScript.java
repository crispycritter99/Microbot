...
public class ThievingLocalScript extends Script {
                ...
                try {
                    ...
                    if (npcStream != null) {
                        wealthyCitizenInteracting = npcStream
                                .filter(x -> x != null && x.isInteracting() && x.getInteracting() != null&&x.getInteracting().getName().equalsIgnoreCase("Leo"))
...
if (Rs2Bank.isNearBank(nearestBank, 8))Rs2Bank.openBank();
        else Rs2Bank.walkToBankAndUseBank(nearestBank);
...
//            Microbot.showMessage(config.food().getName() + " not found in bank");
            Microbot.log(config.food().getName() + " not found in bank");
            sleep(5000);
            return;
        }
