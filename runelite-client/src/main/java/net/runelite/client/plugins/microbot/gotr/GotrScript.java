...
//            Rs2Inventory.dropAll("Guardian essence");
        ...
        if (cell != null && isInMainRegion() && isInMiniGame() && !shouldMineGuardianRemains && !isInLargeMine() && !isInHugeMine()) {
            ...
            if (Rs2Inventory.hasItemAmount(GUARDIAN_ESSENCE, 10)) {
                for (int shieldCellId : shieldCellIds) {
                    ...
                    if (CellType.GetShieldTier(shieldCell.getId()) < cellTier&&Rs2Inventory.hasItemAmount(GUARDIAN_ESSENCE, 10)) {
        ...
        if (!isInMainRegion() && isInMiniGame()) {
            ...
            if (rcAltar != null) {
                ...
                if (Rs2Inventory.hasItem(GUARDIAN_ESSENCE)) {
                    ...
                    sleep(Rs2Random.randomGaussian(Rs2Random.between(600, 1100), 300));
    ...
    private void mineGuardianRemains() {
        ...
        if (Rs2Player.getSkillRequirement(Skill.AGILITY, 56) && getTimeSincePortal() < 85 && (!Rs2Inventory.hasItem(GUARDIAN_ESSENCE)||timeToStart>1)) {
            if (!isInLargeMine() && !isInHugeMine() && (!Rs2Inventory.hasItem(GUARDIAN_FRAGMENTS) || getStartTimer() != -1||timeToStart>1)) {
//                if (!isInLargeMine()) {
                    if (Rs2Walker.walkTo(new WorldPoint(3632, 9503, 0), 20)) {
