...
import net.runelite.client.plugins.microbot.mixology.StrategyHelper;
...
public class MixologyScript extends Script {
            ...
            try {
                ...
                if (isInMinigame) {
                    ...
                    if (Rs2Inventory.hasItem(DIGWEED) && !Rs2Player.isAnimating()) {
                        List<PotionOrder> activeOrdersDigweed = StrategyHelper.getActiveOrders(potionOrders, config.strategy());
                        Optional<Integer> potionItemId = activeOrdersDigweed                                .stream()
                ...
                switch (mixologyState) {
                    ...
                    case MIX_POTION_STAGE_1:
                        ...
                        List<PotionOrder> activeOrdersStage1 = StrategyHelper.getActiveOrders(potionOrders, config.strategy());

                        Map<Integer, Integer> itemsToCheck = new HashMap<>();
                        PotionOrder potionToMake = null;

                        for (PotionOrder _potionOrder : activeOrdersStage1) {
                        ...
                        for (int itemId : itemsToCheck.keySet()) {
                            PotionOrder _potionOrder = activeOrdersStage1
                                    .stream()
                    ...
                    case MIX_POTION_STAGE_2:
                        ...
                        // Sort using a custom comparator, respecting the active strategy
                        List<PotionOrder> nonFulfilledPotions = StrategyHelper.getActiveOrders(potionOrders, config.strategy())
                                .stream()
                        ...
                        break;
                    case CONVEYER_BELT:
                        boolean hasAnyFulfilled = potionOrders.stream()
                                .anyMatch(x -> Rs2Inventory.hasItem(x.potionType().getFulfilledItemId()));
                        if (!hasAnyFulfilled) {
                        ...
                        if (Rs2GameObject.interact(AlchemyObject.CONVEYOR_BELT.objectId())) {
                            ...
                            currentLyePoints = getLyePoints();
                            currentMoxPoints = getMoxPoints();
                            // Reset fulfilled flags so new round can start fresh
                            if (1>2) {
                            potionOrders.forEach(o -> o.setFulfilled(false));

                                boolean allWouldBeSkipped = potionOrders.stream()
                                        .allMatch(o -> StrategyHelper.shouldSkipOrder(o, potionOrders, config.strategy()));

                                if (!allWouldBeSkipped) {
                                    for (PotionOrder order : potionOrders) {
                                        if (StrategyHelper.shouldSkipOrder(order, potionOrders, config.strategy())) {
    ...
    private boolean hasAllFulFilledItems(MixologyConfig config) {
        List<PotionOrder> activeOrders = StrategyHelper.getActiveOrders(potionOrders, config.strategy());
        Map<Integer, Integer> itemsToCheck = new HashMap<>();
        boolean hasAllFulFilledItems = true;

        for (PotionOrder _potionOrder : activeOrders) {
        ...
        for (int itemId : itemsToCheck.keySet()) {
            PotionOrder _potionOrder = activeOrders
                    .stream()
