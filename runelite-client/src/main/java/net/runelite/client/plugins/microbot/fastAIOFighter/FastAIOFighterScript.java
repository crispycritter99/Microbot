...
public class FastAIOFighterScript extends Script {
    ...
    public static boolean lootnet = false;
     boolean test = false;
    private volatile List<String> attackableNpcs = Collections.emptyList();

    public void updateAttackableNpcs(String csvNames) {
        if (csvNames == null || csvNames.trim().isEmpty()) {
            attackableNpcs = Collections.emptyList();
            return;
        }

        List<String> parsedNames = Arrays.stream(csvNames.split(","))
                .map(String::trim)
                .filter(name -> !name.isEmpty())
                .collect(Collectors.toList());
        attackableNpcs = Collections.unmodifiableList(parsedNames);
    }
    public boolean run(FastAIOFighterConfig config) {
