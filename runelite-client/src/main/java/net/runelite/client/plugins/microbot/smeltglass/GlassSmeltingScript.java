...
import net.runelite.client.plugins.microbot.smeltglass.enums.Ores;
...
public class GlassSmeltingScript extends Script {

    static boolean coalBagEmpty;
    static final int coalBag = 12019;
    boolean hasBeenFilled = false;

    public boolean run(GlassSmeltingConfig config) {
...
//                    }
    ...
    private boolean inventoryHasMaterialsForOneBar(GlassSmeltingConfig config) {
    ...
    private void withdrawRightAmountOfMaterials(GlassSmeltingConfig config) {
        for (Map.Entry<Ores, Integer> requiredMaterials : config.SELECTED_BAR_TYPE().getRequiredMaterials().entrySet()) {
