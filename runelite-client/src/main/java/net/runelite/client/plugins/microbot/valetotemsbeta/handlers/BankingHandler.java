...
import net.runelite.client.plugins.microbot.valetotemsbeta.enums.BankLocation;
import net.runelite.client.plugins.microbot.valetotemsbeta.enums.GameObjectId;
import net.runelite.client.plugins.microbot.valetotemsbeta.enums.GameState;
import net.runelite.client.plugins.microbot.valetotemsbeta.enums.TotemLocation;
import net.runelite.client.plugins.microbot.valetotemsbeta.models.GameSession;
import net.runelite.client.plugins.microbot.valetotemsbeta.utils.CoordinateUtils;
import net.runelite.client.plugins.microbot.valetotemsbeta.utils.GameObjectUtils;
...
public class BankingHandler {
    ...
    public static boolean withdrawRequiredItems(GameSession gameSession) {
        try {
            ...
            boolean withdrew = Rs2Bank.withdrawAll(logId);
//            boolean withdrew = Rs2Bank.withdrawX(logId, logsToWithdraw);
        ...
        try {
            if (config != null) {
                switch (config.routeStrategy()) {
                    ...
                    case OAK_TRIANGLE:
                        return TotemLocation.RouteType.OAK_TRIANGLE;
                    case AUTO:
                    default:
                        break;
                }
                if (config.decorationSource() == ValeTotemBetaConfig.DecorationSource.PREMADE) {
                    Microbot.log("Premade decorations selected - using four-and-bank inventory cycles");
                    return TotemLocation.RouteType.STANDARD;
                }
                if (config.logType() == ValeTotemBetaConfig.LogType.OAK) {
                    Microbot.log("Oak selected - using the Wiki 6-7-8 triangle");
                    return TotemLocation.RouteType.OAK_TRIANGLE;
                }
            }
            ...
            Microbot.log("No log basket detected - using the four-totem bank route");
            return TotemLocation.RouteType.STANDARD;
    ...
    public static boolean performUnifiedBankingCycle(GameSession gameSession) {
        try {
            ...
            // Only the full-eight route requires a basket; four-bank and oak use normal banking.
    ...
    private static boolean performStandardBankingOperations(GameSession gameSession) {
        try {
            ...
            if (config != null && config.useBowStringSpool()
                ...
                if (!Rs2Bank.hasItem(InventoryUtils.BOW_STRING_SPOOL_ID)
                        || !Rs2Bank.withdrawOne(InventoryUtils.BOW_STRING_SPOOL_ID)) {
                    Microbot.log("Bow string spool requested but unavailable; continuing unstrung");
                } else {
                    Rs2Inventory.waitForInventoryChanges(3000);
                }
            }
            ...
            // Premade items are intentionally withdrawn as a fixed decoration batch; logs still use Withdraw-All.
            if (config != null && config.decorationSource() == ValeTotemBetaConfig.DecorationSource.PREMADE) {
                int decorationId = InventoryUtils.getBowId();
                if (!Rs2Bank.hasItem(decorationId) || !Rs2Bank.withdrawX(decorationId, 16)) {
                    Microbot.log("Not enough premade decorations for a four-totem route");
                    gameSession.setState(GameState.STOPPING);
                    return false;
                }
                Rs2Inventory.waitForInventoryChanges(3000);
    ...
    private static boolean performExtendedRouteBankingOperations(GameSession gameSession) {
        try {
            ...
            if (config != null && config.useBowStringSpool()
                    && config.logType() != ValeTotemBetaConfig.LogType.REDWOOD
                    && !InventoryUtils.hasBowStringSpool()
                    && Rs2Bank.hasItem(InventoryUtils.BOW_STRING_SPOOL_ID)) {
                Rs2Bank.withdrawOne(InventoryUtils.BOW_STRING_SPOOL_ID);
                Rs2Inventory.waitForInventoryChanges(3000);
            }
     ...
     */
    public static boolean hasSufficientMaterialsForExtendedRoute(GameSession gameSession, int roundsPlanned) {
        try {
            if (!Rs2Bank.isOpen() && !openBank()) {
    ...
    public static boolean depositAllExceptKnifeAndLogBasket() {
        try {
            ...
            boolean deposited = Rs2Bank.depositAllExcept(
                ...
                InventoryUtils.BOW_STRING_SPOOL_ID
            );
     ...
     */
    private static boolean performLogBasketFillingOperation(GameSession gameSession) {
        try {
