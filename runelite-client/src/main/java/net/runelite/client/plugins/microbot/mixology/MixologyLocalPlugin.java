...
@PluginDescriptor(
        name = PluginConstants.MOCROSOFT + "Mixology Local",
        description = "Mixology plugin",
        authors = { "Mocrosoft" },
        version = MixologyLocalPlugin.version,
...
@Slf4j
public class MixologyLocalPlugin extends Plugin {
    ...
    private void updatePotionOrders() {
        ...
        boolean allWouldBeSkipped = mixologyScript.potionOrders.stream()
                .allMatch(o -> StrategyHelper.shouldSkipOrder(o, mixologyScript.potionOrders, strategy));

        if (!allWouldBeSkipped) {
            for (PotionOrder order : mixologyScript.potionOrders) {
                if (StrategyHelper.shouldSkipOrder(order, mixologyScript.potionOrders, strategy)) {
