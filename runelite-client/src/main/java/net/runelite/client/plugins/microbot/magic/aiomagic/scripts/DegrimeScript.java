...
import net.runelite.client.plugins.microbot.degrime.DegrimeCurrentStatus;
            ...
            try {
                ...
                switch (state) {
                    case BANKING:
                        ...
                        Rs2Bank.depositAllExcept(ItemID.RUNE_POUCH,ItemID.NATURE_RUNE,ItemID.EARTH_RUNE);
                        ...
                        if (!Rs2Inventory.hasItem(plugin.getHerb())) {
                            if (!Rs2Bank.hasItem(plugin.getHerb())) {
                                Microbot.showMessage("Herb not found");
                                shutdown();
                                return;
                            }

                            Rs2Bank.withdrawAll(plugin.getHerb());
...
//                        sleep(Rs2Random.randomGaussian(300,50));
                        ...
                        break;
                    case CASTING:
                        if (!Rs2Inventory.hasItem(ItemID.RUNE_POUCH)&&!Rs2Inventory.hasItem(ItemID.NATURE_RUNE)) {
    ...
    private boolean hasRequiredItems() {
        if (Rs2Inventory.hasItem(plugin.getHerb())) {
            return Rs2Inventory.hasItem(plugin.getHerb());
        }
        return false;
    }
    }
