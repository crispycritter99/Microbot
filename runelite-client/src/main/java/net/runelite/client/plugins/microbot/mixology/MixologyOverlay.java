...
public class MixologyOverlay extends OverlayPanel {
    private final MixologyLocalPlugin plugin;
    private final ModelOutlineRenderer modelOutlineRenderer;

    @Inject
    MixologyOverlay(MixologyLocalPlugin plugin, ModelOutlineRenderer modelOutlineRenderer) {
        this.plugin = plugin;
    ...
    public Dimension render(Graphics2D graphics) {
        ...
        panelComponent.getChildren().add(TitleComponent.builder()
                .text("Micro Mixology V" + MixologyLocalPlugin.version)
                .color(Color.GREEN)
                .build());
        ...
        for (MixologyLocalPlugin.HighlightedObject highlightedObject : this.plugin.highlightedObjects().values()) {
